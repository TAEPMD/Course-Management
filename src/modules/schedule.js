...
import { FISCAL_MONTH_NAMES, getFiscalMonthRange, isFiscalMonthInRange } from '../utils/courseMetrics.js';

const MONTH_NAMES = FISCAL_MONTH_NAMES;
const Q_LABELS = ['Q1','Q1','Q1','Q2','Q2','Q2','Q3','Q3','Q3','Q4','Q4','Q4'];

function getFiscalMonthNumber_(date = new Date()) {
  return ((date.getMonth() + 3) % 12) + 1;
}
...
export function renderFiscalCalendar() {
  ...
  const todayMonth = getFiscalMonthNumber_();
  grid.innerHTML = '';

  MONTH_NAMES.forEach((name, i) => {
    const m = i + 1;
    const inRange = isFiscalMonthInRange(m, start, end);
    ...
    btn.className = `relative py-3 rounded-2xl text-sm font-bold transition-all duration-200 border-2 cal-month-btn ${
      inRange
        ...
        : 'bg-white text-gray-600 border-gray-100 hover:border-blue-300 hover:bg-blue-50'} ${isToday ? 'ring-2 ring-amber-400 ring-offset-2' : ''}`;
    btn.innerHTML = `
      <span class="block text-[10px] font-black opacity-60 mb-0.5">${Q_LABELS[i]}</span>${name}
      ${isToday ? '<span class="absolute top-1 left-1.5 text-[8px] font-black text-amber-500 bg-amber-100 px-1.5 py-0.5 rounded-full">Today</span>' : ''}
...
function _updateCalendarLabels(start, end) {
  const duration = getFiscalMonthRange(start, end).length;
  ...
  if (el('cal-duration-label')) el('cal-duration-label').textContent = duration;
}
